...
tmp.data <-
    read_tsv (
        file = 'data/raw/TAB_NGA10068.tsv', # change here
        trim_ws = TRUE,
        skip = 24,
        progress = FALSE,
...
savedas = 'NGA10068.rds' # change here
...
dvars <- selected_vars[!(selected_vars %in% var.names)] # find the missing columns

tmp.data <- tmp.data %>% `is.na<-`(dvars) # add the missing columns

pre.data <- tmp.data[, selected_vars]
names(pre.data) <- study.vars
